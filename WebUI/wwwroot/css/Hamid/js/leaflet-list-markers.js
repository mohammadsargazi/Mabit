...
L.Control.ListMarkers = L.Control.extend({
	...
	onAdd: function (map) {
		...
		//var container = this._container = L.DomUtil.get('list-mabit');

		//this._list = L.DomUtil.create('ul', 'list-markers-ul', container);
		//this._list = L.DomUtil.get('list-parent');
		this._list = L.DomUtil.get('list-page-main-items');
	...
	/* _createItem: function(layer) {
		...
		{
			a.innerHTML = icon+'<span  title="'+layer.options['test']+'" >'+layer.options[this.options.label]+'</span> <b>'+this.options.itemArrow+'</b>';
